...
LANES=(
  "policy|公开模式主干|scripts/demo_e2e.py|LangChain 流式(含早停) + MCP 工具(参数/结果) + 真 SP1 证明 + 挑战绑定 + 账本"
  ...
  "anchor|链上锚定(P7-c)|scripts/anchor_e2e.sh|起 anvil → 部署 Anchor.sol → 12 张证书摘要上链 → 第三方 --rpc 核对"
