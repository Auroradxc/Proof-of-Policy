...
|---:|---:|---:|---:|---:|---:|---:|
| 500 | 27 ms | 2 ms | 79 ms | 46 ms | 7 ms | 144 ms |
| 1000 | 100 ms | 3 ms | 288 ms | 175 ms | 23 ms | 568 ms |
| 2000 | 393 ms | 6 ms | 1,138 ms | — ⚠️ 越界（>1500） | 83 ms | 2,261 ms |

## 每字符成本（ms/char）—— **看这一张**

| L | agent_content | agent_tool | eu_ai_act | finance_redaction | multiparty_demo | pii_redaction |
|---:|---:|---:|---:|---:|---:|---:|
| 500 | 0.0548 | 0.0036 | 0.1578 | 0.0919 | 0.0136 | 0.2889 |
| 1000 | 0.1002 | 0.0029 | 0.2877 | 0.1752 | 0.0230 | 0.5683 |
| 2000 | 0.1965 | 0.0028 | 0.5688 | — | 0.0417 | 1.1306 |
...
- agent_content：L ×4.0，每字符成本 0.0548 → 0.1965 ms/char（×3.59）；拟合常数 0.219 → 0.196 µs/char²
- agent_tool：L ×4.0，每字符成本 0.0036 → 0.0028 ms/char（×0.77）；拟合常数 0.015 → 0.003 µs/char²
- eu_ai_act：L ×4.0，每字符成本 0.1578 → 0.5688 ms/char（×3.60）；拟合常数 0.630 → 0.569 µs/char²
- finance_redaction：L ×2.0，每字符成本 0.0919 → 0.1752 ms/char（×1.91）；拟合常数 0.367 → 0.350 µs/char²
- multiparty_demo：L ×4.0，每字符成本 0.0136 → 0.0417 ms/char（×3.06）；拟合常数 0.054 → 0.042 µs/char²
- pii_redaction：L ×4.0，每字符成本 0.2889 → 1.1306 ms/char（×3.91）；拟合常数 1.153 → 1.130 µs/char²

拟合常数 `c = 总耗时 / (L(L+1)/2)` 在各 L 上基本不变 —— 说明 `总耗时 ≈ c·L²/2` 这个模型站得住，二次性是量出来的、不是外推的。

**包间差也是真的**：同为 2000 字符，最贵的 `pii_redaction` 与最便宜的 `agent_tool` 差 **405×**（1.131 vs 0.0028 ms/char）。因为每个采样点要对**完整前缀**跑一遍**全部** pattern 的 NFA。所以「流式要多久」**没有单一答案，必须连着策略包说**。
