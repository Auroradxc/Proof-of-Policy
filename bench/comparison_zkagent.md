...
**验证时间**：纯验证 ~90 ms（vkey setup 1.5 s 一次性；CLI 冷启动 ~20 s，含证明器构造）。

zkVM 执行周期数：100k 字符 × 1 规则 ≈ **7.44×10⁶**；× 3 规则 ≈ **4.29×10⁸**；× 6 规则
≈ **5.41×10⁸**。固定规则集时对长度**精确线性**（R² ≈ 1.0000）。

⚠️ **本表的规模上界受内存而非周期数约束**：SP1 `core` 证明的峰值常驻有一条 ~10.15 GiB 的
**固定地板**（最小配置就已 10,389 MB），本机 12 GB 上只剩很窄的缝 —— 可行域是**两级台阶**
（1 条规则 ≤10k 字符可证、2 条规则约 200 字符可证、3 条及以上 OOM），不是一条曲线。
周期表能扫到 100k 字符 × 6 规则，因为那只跑执行不出证。见 `results/proofs.md`。
...
| 证明规模/上下文 | GPT-2，T=512 | 同左 | 200–10,000 字符 × 1–2 规则 |
| 证明时间 | 194.40 s | ~1069 s | **118.9–172.5 s**（固定开销主导） |
