...
| `--out-dir` | `scripts/examples/out/service` | 作业、证书、账本的落盘根。作业记录在 `<out-dir>/jobs/<job_id>/job.json`，见 §1.5 |
...
每个作业在 `<out-dir>/jobs/<job_id>/` 下有自己的目录，里面两类东西：
**产物**（`payload.json` / `cert.json` / `proof.bin` / `receipts.json`）与一份
**索引** `job.json`。进程怎么没的都不影响这一点：重新起服务之后
`GET /v1/attest/{job}` 照常答得出来，**已经完成的作业不需要重跑证明**。

**停服务用 `Ctrl-C`（`SIGINT`），别用 `SIGTERM`**：`SIGINT` 走
`stop(drain=True)`，会等手上的作业做完；`SIGTERM`（`systemd stop` / `docker stop`
/ 裸 `kill`）**没有装处理器**，进程当场就没了 —— 记录里留下的就是最后那一笔
`proving`，重启后如实报 `failed`（「服务在它跑完之前重启了」）。磁盘上的产物
不受影响，代价只是那条作业得重新提交。

| 写盘时机 | 为什么 |
|---|---|
| 入队（`queued`） | 让这个 id 在重启后不是 404，而是一句说得清的「它没跑完」 |
| 开工（`proving`） | `proving` 是**唯一**一段「进程被杀之后，磁盘上还来不及说它没跑完」的窗口。不落这一笔，重启后读到的还是 `queued`，「它到底开始过没有」就没人答得上来 |
| 终态（`done`/`failed`） | 结果本身。放在 `finally` 里，异常路径也落 |

**重建时以磁盘为准**（三条规则，见 `_load_job`）：

- **有证书就是 `done`**，哪怕记录停在 `proving` —— 崩溃可能恰好落在「证书写完」
  与「改记录」之间。反过来会让系统声称「没有证书」，而它明明躺在那儿。
- **中间态不接回来当中间态**：`queued`/`proving` 而磁盘上没有证书 ⇒ 报 `failed`
  并明说「服务在它跑完之前重启了，它**不会**再被执行，也没有证书」。
  接着跑一个半截作业只会产出更可疑的东西；回一句「还在跑」则是撒谎。
- **记录说 `done` 但证书不见了** ⇒ 报 `failed` 并明说「没有证书可以交付，
  账本里可能仍有它的锚定记录」。这一条最容易被**清理动作**触发：删了产物目录
  就不要再指望这条作业答 `done`。

**记录与产物对不上就叫失败**，不挑一个信：记录里的 `cert_digest` 与磁盘重算的
不一致（有人改了 `payload.json`）、或 `cert.json` 里的载荷与 `payload.json`
不是一个东西（只改了其中一个）—— 两种都报 `failed`，`error` 里点名是哪一种。
**产物读不出来或形状不对也走同一条路**（`cert.json` 是个合法 JSON 但不是该有的
样子、被截断、被别的程序覆盖）：一样返 `failed` 并把异常类型写进 `error`。
**读取方自己绝不崩** —— 这一格原先会漏成 HTTP `500`（2026-09-13 补测查出：
形状错的信封让读回证书那段抛 `TypeError`，而捕获列表里没有它）。

**写入是原子的**（`tmp` + `os.replace`）：崩在写一半只会留一个 `.tmp`，读到的
还是上一份完整记录。写盘失败**不会让作业失败**，只在 stderr 上喊一句 ——
证书比索引重要；但也不会静默，那个文件存在的唯一理由就是重启后还查得到。

**`job_id` 先按形状过一遍**（`^job-[0-9a-f]{16}$`）再去碰文件系统：`GET` 的路径
段是外部输入，形状不对的直接当作「不存在」，连目录都不去凑。

**提交者也落盘**（`owner`），所以重启后「别人的作业返 404」这条规则还成立 ——
见 §1.3。运维要按提交人查历史，`job.json` 就是那份材料。
...
| **重启后查旧作业** | **200**，从磁盘重建 | 产物是权威、`job.json` 是索引，见 §1.5。**服务重启不再是「查不到」** |
| 记录与产物对不上（摘要/信封） | `state: failed` + 点名是哪一种 | 不挑一个信。见 §1.5 |
| 产物坏掉（读不出来 / 形状不对） | 同上，**不是 500** | 读取方不许自己崩 —— 它一崩，接口就答不出「到底怎么了」。见 §1.5 |
...
2. **语义规则策略不受支持**。`semantic_bound` 由 ezkl 陪伴证明判定，而该证明只
   ...
   但在那之前它看起来是正常的）。
3. ~~**作业状态只在内存里**~~ **已闭（作业状态持久化，2026-09-13）**。见 §1.5：
   作业记录落 `<out-dir>/jobs/<job_id>/job.json`，重启后 `GET /v1/attest/{job}`
   照常答得出来；**仍然不设防的两件事**如实留下：
   (a) **记录没有保留期** —— 清理 `<out-dir>/jobs/` 等于放弃历史查询：删掉整个
   作业目录 ⇒ 该 id 重新变回 404，只删产物 ⇒ 那条作业如实改报 `failed`
   （「没有证书可以交付」）。要留档就别删，或者把 `--out-dir` 整体归档；
   (b) **查询按 id，没有列表接口** —— 没有 `GET /v1/attest` 这样的「列出我的
   作业」，要按提交人翻历史得自己去读 `job.json`。
4. **证明作业不可中断**。`SIGINT`/`Ctrl-C` 会等手上的作业做完再退（缺省
   `drain=True`）；一个已经进入 `proving` 的作业不能取消。`SIGTERM`/`SIGKILL`
   则是**当场进程就没了**（前者没装处理器，见 §1.5），留下一个半截目录 —— 它的
   证书不会签发，账本也不会多出一条，所以**不会变成假证据**；重启后那条作业
   如实报 `failed` 并说明「它不会再被执行」（§1.5 规则 2）。
...
| 读不到某个作业（404）但产物在磁盘上 | 三种可能，按这个顺序排：①那个作业不属于这把 token（重启后依然如此，提交人记在 `job.json` 的 `owner` 里）⇒ 用 `--auth-admin` 的标签去问；②`job.json` 还在，只是没读出来（损坏/字段非法）⇒ 那条会答 `failed` 并说明原因，不是 404；③**整个作业目录被清掉了** ⇒ 真的 404（见 §5.3） |
| 重启后旧作业答 `failed`，`error` 说「不会再被执行」 | 正常行为：它当时停在 `queued`/`proving`，且磁盘上没有证书 —— 半截作业不会被接着跑（§1.5）。**重新提交**会得到一个新的 `job_id`；如果账本里已有这个作业的锚定摘要，说明证书当时签出来了，去看 `<out-dir>/jobs/<job_id>/cert.json` |
