...
>
> **这台机器还要顺带补 P2-12 的证明侧全矩阵**（64 GB 对 P2-12 是过剩的，跑 core 全矩阵
> 只是余量利用，约几十分钟机时）—— 定位是「顺带」而非第二个阻塞：**做不完不影响本项收尾，
> 也不需要为它单独租机**。顺序、内存粗估与跨机口径见 **§9 待办 T1 的「第二个用途」**，
...
**剩余采样（不阻塞，已并入 §9 待办 T1 顺带完成）**：本机的 6 个点是**边界**不是全矩阵 ——
「20k 字符 × 6 规则」那一格从未真出过证，本机也补不了（见上「天花板是内存」）。
这份缺口**顺带**挂在 T1 的租机窗口里（那台机器是为 P1-7 的 groth16 租的，内存需求
高得多，跑 core 全矩阵属于余量利用）：到机后按 **① P1-7 groth16 → ② 全矩阵
（20k/50k/100k × 1/2/3/6，`core`）→ ③ `compressed` 对照**的顺序做。
**它不是 T1 的阻塞项，做不完不影响 P1 段收尾，也不需要为它单独租一次机**；
优先级、内存粗估（~16 GB，**未实测**）与跨机口径都在 **§9 待办 T1 的「第二个用途」**里。
...
> **2026-09-13 补**：在 T1 之外，另有一条**代码侧推得动**的延伸路线已立项并写进
> [`dev-plan.md`](dev-plan.md) §5 —— **第一步「接真实 agent」**（统一网关身份 /
> `--model` 真模型 / 补 `on_llm_error` / 真早停 / MCP `list_tools()`）+ **第二步
> 「证明服务」**（`/v1/check` 毫秒级 + `/v1/attest` 队列出证 + `/v1/attest/{job}`
> 轮询，只用标准库）。二者与 T1 **互不阻塞**，也不受 T1 影响：T1 补的是链上那一格，
> 这两步补的是「把已有能力接到真实世界」。
> 同处还记录了本轮复盘查出的第三个「说了但没接上」：
> `demo_e2e.py` 的魔法 `vkey = "demo"` —— 结论与修法见 `dev-plan.md` §5.3。
...
#### T1 的第二个用途：把 P2-12 证明侧的**全矩阵**顺带补完

P2-12 在本机交出来的是一条**边界**而非整张矩阵（见 §P2-12）：`(20000,1)` 与 `(200,3)`
被 OOM 杀，所以「20k 字符 × 6 规则」那一格从来没有真出过证。**这个缺口不是本机调参能
补的**——它卡在内存上（`SHARD_SIZE=262144` 等 prover 选项实测无效）。

但它**天然搭 T1 的车**，理由是那台机器**已经在租了**，而这两件事的硬件要求差得很远：

| | 内存需求 | 依据 |
|---|---|---|
| **P1-7（groth16 + 链上验证）** | **≥64 GB** | **实测**：compressed 与 groth16 分别在本机峰值 11.0 / 11.07 GB 被 OOM 杀，**递归包装的固定开销本身就超了 12 GB** |
| **P2-12 全矩阵（core）** | **约 16 GB**（**粗估，未实测**） | 由本机实测外推：`core` 的固定地板 ~10.15 GiB，加长度/加规则数都只在这条地板之上窄幅浮动（200×1 → 10k×1 全程只涨了 117 MB）。所以「2 倍地板」是给 100k×6 留的余量，**不是拟合出来的数**——真值只有在那台机器上量了才知道 |

⇒ **64 GB 对 P2-12 而言是严重过剩**，它是被 P1-7 拉上去的。正因为过剩，把全矩阵挂在
同一次租机窗口里跑**几乎不增加什么成本**（几十分钟的机时），却能一次填掉本机补不了的
那几格。所以这件事在 T1 里的定位是**「顺带」而不是「第二个阻塞」**：T1 到点后先做
P1-7，P2-12 的全矩阵排在后面，**做不完也不影响 P1 段收尾、也不需要为它单独租一次机**。

**到机后的优先级排序**（一次窗口、按此顺序，前面做完再做后面）：

1. **P1-7：groth16 出证 + 验证合约测通** —— 唯一的硬需求，**必须保住**；
2. **P2-12 全矩阵**：`20k / 50k / 100k` × `1 / 2 / 3 / 6` 规则，**全部 `--proof-mode core`**。
   本机那 6 个点用**同一套采样参数**重跑一遍，好让两张表在同一口径下拼接
   （云机数字与本机数字**不可直接比较**，见下）；
3. **`compressed` 对照**：在**少数几个点**上跑 `--proof-mode compressed`，量「递归包装
   比 `core` 贵多少」——这正是本机因 OOM 完全量不到的一格，也是
   [`bench/comparison_zkagent.md`](../bench/comparison_zkagent.md) §4.4 里
   「`groth16`/`plonk` 是唯一可能隐藏见证的模式，但需 ≥16 GB 内存，本机未实测」
   那句话的补测。

⚠️ **跨机口径**：云机那批数字**不能与本机 12 GB 的表并列读数**。SP1 的证明耗时随 CPU
核数与型号走、可行域随内存走（同一组采样点换台机器就是另一张表）。因此：

* `bench_proofs.py` 现在会把**机器信息（核数 / CPU 型号 / 内存 / hostname）与 `proof_mode`
  一并写进 `results/proofs.json`**，并在 `.md` 顶部打出来 —— 就是为了让云机的结果
  **自带硬件标注**，否则那批数入库后无从解释；
* 入库时**另起一份结果文件**（如 `proofs-cloud.json` / `proofs-cloud.md`），
  与本机的表**并列呈现**而不是合并，并显式标注硬件差异；
* 论文里若引用跨机数字，必须**连着机器一起引**（本机已按这个口径写：
  「本机 12 GB / 24 核」与「云机 N GB / M 核」分开陈述）。

**具体的云机命令**见 [`reproduce.md`](reproduce.md) 的「云机 runbook」一节。
