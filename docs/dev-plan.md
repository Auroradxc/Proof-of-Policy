...
630 例 / 29 个 `policydsl` 模块 / 22 个脚本 / 3 个 guest，链路每一格都接上了；
...
`docs/plan-p0p1p2.md` 的 `469` **不改成 630** —— 它是 P0/P2 阶段的
...
那两篇 dated 快照的抬头到位；全套测试仍全绿（纯文档改动，预期 630/15 不变）。
...
**验收 P3**：`GenericGuard` 有单测覆盖（生成路径 / 工具路径 / seal 与截尾 /
一把网关跨路径共享 / 证书能被 `verify_session.py` 独立验过）；文档里的清单表
每一行都能在三个适配器里指出**对应实现**（写不出对应实现的行就是编的，删掉）。
