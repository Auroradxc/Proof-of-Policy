...
7. **给 `PoPCallbackHandler` 补 `tool_monitor`**：让工具事件走工具策略，
   与同仓的 `LangGraphEventCertifier` 对齐。**已做（`aacc46d`，2026-09-18）**
   ...
   **落地记录（`aacc46d`，实测）**：全量 **791 OK / 15 skip**（改动前 783/15，
   新增 6 条离线 + 2 条真实框架）。端到端证据是接入测试 demo 本身（`9c161f1`）：
   两个按事件过滤的子类**已删**，改成一个 handler 直连，三场景全绿、
   `verify_session.py` A/B/C 全 PASS，且**回执条数与工具证书数重新对上**
   （A 1/1、B 1/1、C 0/0；修前一次工具调用被两个 `on_tool_end` 各签一条回执）。
   `06-frameworks.md` §8.2 第 2 条同步改成 `tool_monitor.on_tool_call`，
...
| 5 | **demo 文档重写**（第 9、11 条）：补 `make_shots` 一环、修环境前提、统一计时口径、更新产物树；**重渲**三份过期产物 | 8 条支路与 `LANES` 机械比对一致 + 四份产物 mtime 全部更新 | ✅ 2026-09-18 |
| 6 | **开发手册校对**：`docs/development.md` 补两条 verify 脚本与 `--shots` 用法，计数对齐 | 手册里每条新命令**实敲一遍** | ✅ 2026-09-18 |
| 7 | 收尾：测试计数「四处同步」、交叉链接普查、推送 | 全量 + 工作树干净 + origin 同步 | ✅ 2026-09-18 |
...
##### 步 6 的留痕（两条，都是「复敲才发现」的）

1. **`loader_parity.py --verify` 裸跑会 `exit 3`** —— 手册里此前**一个字都没提**这个脚本
   （`grep` 在 `development.md` 里 0 命中），补上时才发现它的 CLI **故意不做** R7 那处
   申报归一的折算：快照采于收敛之前，8/11 份加载器把 `description` 丢成 `''`（复敲时
   逐份点数确认：11 份里 8 份为空、3 份带值，与 §5.7.12 记的一致），收敛后的统一加载器
   按包内声明填回。折算写在 `tests/test_loader_parity.py::normalized_golden()` 里，
   常驻闸门是那 11 例测试。**手册如实写「裸跑是红的，且红得正确」** —— 比把它藏起来、
   让人第一次敲就以为改坏了要好。
2. **`--shots` 的产物分两处落**：`demo_all.sh --shots` → `<out>/shots/`（不入库），
   `make_shots.py` 的缺省 → `docs/demo/`（**入库**）。`docs/development.md` §7 的产物地图
   此前只有一条「`demo_*.py` → 否」的口径，入库的那份漏了。

**耗时复敲**（2026-09-18，与前一轮的差值都记进 `development.md` 表里）：
`unittest discover` 40.6 s（783/15）· `demo_all.sh` **13.5 s**（3.3 表里原写 13.2）·
`--shots` 14.2 s · `demo_e2e.py --no-prove` **1.5 s**（原写 1.16）·
`verify_session.py` **0.084 s**（原写 0.081，10 项全 PASS）·
`acceptance.py --verify` **1.1 s**（原写 1.4）· `test_loader_parity` 0.54 s（11 例）。
§3.1 的代码块注释还写着 `# 33.4 s`、与同一页表里的 40.6 s 自相矛盾 —— 已改。
手册抬头加了「±10% 是常态，别拿个位数抠」这句，免得下一轮又把噪声当漂移来修。

##### 步 7 的留痕：本轮重闸门（真出证）与收尾普查

**① `bash scripts/demo/demo_all.sh --prove` —— 8 条支路全 PASS，`SKIP` 集合 = ∅。**

严格串行跑完一轮（`setsid nohup … & disown` 脱离，全程只挂一个 `kill -0` 轮询的等待进程）。
`REPORT.md` 里 `SKIP` 只出现 **1 次**，且是**图例那句**（「SKIP = 缺依赖…」），
不是被跳过的支路 —— 「SKIP 集合 = ∅」这个判据要这么核，不能只 `grep -c` 数个数。

| 支路 | 墙钟(s) | 峰值(KiB) |
|---|---:|---:|
| 公开模式主干 | 136.0 | 10,627,636 |
| 私有模式 | 131.7 | 10,744,136 |
| 语义规则(P2-9) | 10.6 | 1,049,228 |
| 组合证明(P1-6) | **383.6**（本轮最贵）| 10,745,608 |
| 会话聚合(P2-10) | 290.5 | **10,888,728**（本轮最高，≈10.38 GiB）|
| 多证明者(P2-11) | 210.1 | 10,868,904 |
| 链上锚定(P7-c) | 169.3 | 10,808,584 |
| 第三方独立验证 | 25.9 | 7,292,868 |

墙钟 **22.6 min**（09:22:59 → 09:45:36），与逐条之和 **1357.7 s** 吻合 —— 八条严格串行，
墙钟 ≈ 逐条相加，这个吻合本身就是「没有并行」的旁证。语义支路仍只有 **1.0 GiB**
（走 ezkl，不碰 SP1 证明器），与 §5.7.22 记的一致。

**② 一处实测偏差：`--prove` 的墙钟从「26–27 分钟」改成「22.6 分钟」。**
仓库里 **7 处**（`README.md` / `scripts/demo/demo_all.sh` 用法头 / `docs/demo/README.md` /
`docs/reproduce.md` / `docs/modules/07-cli-scripts.md` ×3）此前都写「本机实测 26–27 分钟」。
这次实测 22.6 min，差 **15%**，超出刚在 `development.md` 抬头写下的 ±10% 口径，
所以按实测值改掉，并在 `docs/demo/README.md` §2 留了一句「这是个**量级**不是常数」，
把「更早一轮读到 26–27 分钟」一并记下 —— 只留新数字会让人以为是回归。
注意这与 §5.7.22 的 20.9 min 也不等：同一命令三次读数 20.9 / 22.6 / 26–27，
出证耗时的浮动比测试计数大得多，**这类数字别当判据用**。

**③ 交叉链接普查：47 份 `.md`、仓库内相对链接 291 条、悬空 0。**
用仓库**自己的**判据跑（`tests/test_doc_links.py` 的 `_iter_links` / `_is_repo_path`），
不是另写一个正则 —— 自写的第一版报了 2 条「悬空」，逐条核对**都是假阳性**：
`docs/dev-plan.md:1188` 与 `docs/plan-p0p1p2.md:550` 里的 `Pr[...](...)` 是**论文式概率记号**，
仓库的 `_is_repo_path` 本来就把它排除在外。**这正说明为什么该复用判据而不是重写** ——
重写出来的那份会把噪声当信号，然后你要花时间去核一个不存在的问题。

同类的一笔：`docs/refactor-proposal.md:149` 记着「§5.6.9 那 268 条链接的普查脚本是
**一次性的，没入库**」。**这个缺口现在已经补上** —— `tests/test_doc_links.py` 就是那个普查的
常驻版，跟着 `unittest discover` 走，还有一条「扫到的文档数 ≥ 40」的守卫防止它退化成
「一份文档都没读、一条链接都没找到」式的全绿。`dev-plan.md:1188` 那个 **268** 是
**某一阶段**的闸门表，按本轮约定（阶段验收表是留痕）**不动**；291 这个当前值记在这里。

**④ 测试计数「四处同步」复核通过。** `783 / 15` 在五个文件里逐处对得上：
`docs/modules/08-tests-bench.md`（§1 表 + 合计行 + 顶部「42 个模块」+ §5 验收判据）、
`docs/modules/README.md`（目录树 / 板块表 / §5 命令）、`docs/security-model.md:569`、
`docs/reproduce.md`（§1 命令 + §验收），以及 `docs/development.md`（§3.1 表 + §4.2 清单）。
每处的日期括注都带上了「2026-09-18 §5.8 步 5（demo 支路表闸门，+1 例）后重测」。
