...
### 5.7 重构优化建议书（2026-09-17 立，**提案，未采纳**）
...
| **R17a** | `pii_redaction_v1` / `agent_tool_v1` 补 `length_bound` | 7 个包重跑 `check`/`compile`，结论逐条不变 | 这两个包是**仅有的没有长度上界**的，而 `pii_redaction_v1` 是**全部 7 个包里最贵的**（✅ 2000 字符 4.9 s、5000 字符 29.9 s，二次增长）——**没有上界 = 代价没有天花板** |
