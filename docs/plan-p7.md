...
> ⚠️ **这是 P7 阶段（2026-09-10）的按日工作记录，不是现状。** 本文里的测试计数
> （如 A 的「4 例」、B 的「8 例」）是**当时的计划/快照**，后续阶段又加了用例，
> 故与今天对不上 —— 这是**如实保留的历史**，不要改。**测试数量的现状一律以
> [`docs/modules/08-tests-bench.md`](modules/08-tests-bench.md) 为准**（它一直拿
> 真跑的数更新）。理由与同一批订正见 [`dev-plan.md` §5.4.1](dev-plan.md)。
