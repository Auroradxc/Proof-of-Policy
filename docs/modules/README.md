...
> 上一层入口：[`docs/README.md`](../README.md)（全部文档的索引与状态标注）。
>
> 本目录按**功能板块**拆解 zk-policy 的实现：每个板块一份文档，包含
> 「职责 / 文件清单 / 关键数据结构 / 函数级 API / 不变量与边界 / 测试对应 / 扩展指引」。
> **每篇最后另有两节，结构与前面不同**：**「怎么用它」**（我该怎么调它，附可照抄的命令与代码片段）
> 与**「怎么改它」**（我要改这个板块时改动落在哪、改完必须跑哪几条验证）。前七节回答
> 「它是怎么实现的」，这两节回答「我拿它干活 / 我要动它的时候怎么办」。
...
│   ├── __init__.py               #   门面：re-export Policy / compile_policy / check / 工具回执链
│   ├── __main__.py               #   CLI：compile / check
│   ├── paths.py                  #   仓库根的唯一出处（find_repo()，按标记目录向上找）
│   ├── core/                     #   策略 DSL 与参考判定
...
├── tests/                        # 单测与集成测试（783 passed / 15 skip）
...
> ①–⑦ 是「跑一次、拿一份产物」。要把它们做成**常驻进程**，见
> [`07-cli-scripts.md` §2.14 `proof_service.py`](07-cli-scripts.md)：两段接口
> （`/v1/check` 毫秒级出 unproven 证书 / `/v1/attest` 入队出真证明）+ 作业队列，
> 库在 `policydsl/runtime/service.py`。

---

## 4. 板块索引

> 「覆盖文件」一列写的是**相对 `policydsl/` 的路径**（`circuits/` 等除外），
> 子包名在前、模块名在后，和仓库里的真实位置逐字对应 —— 要跳转就直接照抄。
...
| 08 | [测试与评测](08-tests-bench.md) | `tests/*` `bench/*` | 783 个测试覆盖什么、评测数字怎么来的 |
...
| I6 | **策略绑定**：证书声称的 `policy_hash` == 由策略包现场重编译的 == **证明公开值承诺的**（三方比对，缺一不可；只有两个来源时一律判失败，防空洞） | `policydsl.evidence.verifier.check_policy_binding`；`tests/test_policy_binding.py` |
| I7 | **响应绑定**：证书 `challenge` 块声明的 `response_binding` == outcome 内嵌的 == **证明公开值承诺的** == 由**送达的响应 T′** 与 nonce 现场重算的（同样 ≥2 来源才算比对）。绑定公式含 nonce 长度前缀，`(nonce, T) → 字节串`恒为单射 | `policydsl.evidence.verifier.check_response_binding`、`commit.response_binding`（↔ `pop_types::response_binding`）；`tests/test_binding.py` |
...
```bash
# 只跑参考层（秒级，无需 Rust）
python3 -m unittest discover tests -v            # 783 passed / 15 skip
