...
>
> ⚠️ **但这只是「公开值里没有明文」，不是「`T` 不可恢复」**（P0-4 查证的结论，见
> [`../security-model.md`](../security-model.md) §5.2 与 [`../sp1-zk-audit.md`](../sp1-zk-audit.md) §4）：
> `core` / `compressed` 证明**并非零知识**，且 `b` 与 `c` 都是公开、可离线重算的 `T` 的函数 ——
> 对**低熵** `T`（自然语言响应），验证者枚举候选即可用**猜测—验证**把响应还原出来。
> 更关键的是 L2 **要求** `b` 可被任意持 `(n, T′)` 的一方重算（这正是它防「换响应」的价值），
> 所以在「验证者独立重算绑定」这一前提下，**「响应绑定」与「响应内容隐藏」对低熵 `T` 互斥**。
> 准确的对外表述是「**策略零知识**（合规性可证而不暴露违规内容）+ 响应内容隐藏有明确上界」，
> 证书用 `binding.proof_mode` 标注该工件是否具备隐藏（`cert.proof_hiding()`）。
