...
> **R9(b) 之后，这条二次性对「不扫文本的包」在小 L 上被压到了噪声以下**
> （2026-09-17）。`canonical_violations` 原先**无条件**先算两个 O(L) 的派生量
> （`ascii_lower` 与 `token_count`），而流式路径对每个前缀各调它一次 ⇒ 又是一份
> `Θ(L²)`，且在 `agent_tool_v1` @2000 上占了 110 ms 里的 **101 ms**。改成按需计算
> 之后：`agent_tool` 2000 字符 **110 → 6 ms（19.7×）**、`multiparty_demo` 131 → 81 ms
> （1.4×），而**含 `keyword_block` 的四个包几乎不动（1.0×）** —— 那里的 `lower`
> 本来就真被读到，省不掉。
>
> **二次项没有消失**（参考评估器仍然逐个前缀重扫），只是一个常数被拿掉了：
> `agent_tool` 的拟合常数 `c` 从 0.055 掉到 0.003 µs/字符²，在 L ≤ 2000 这个区间里
