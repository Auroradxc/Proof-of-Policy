...
> 覆盖 `tests/`（27 个模块，630 个用例）与 `bench/`（5 个脚本，结果入库在 `bench/results/`）。
...
```bash
python3 -m unittest discover -s tests -t . -v   # 期望 630 passed, 15 skipped
```
...
- **更新论文数字**：跑完 `bench_*.py` 后，`README.md`、`paper/proof-of-policy.md` §7、
  ...
  当前验收判据是 **630 passed / 15 skip**（2026-09-13 复跑；CI 上更多 skip，见 §1）、
