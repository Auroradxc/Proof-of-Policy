...
class PoPCallbackHandler(BaseCallbackHandler):
    ...
    **跨 provider 不可比**，它变成了「抢在几个 chunk 内」而不是「抢在几个字符内」。
    ...
    而前缀长度随采样点线性增长 —— 实测 L 长 4 倍、每字符成本也长 ~3.6–3.9 倍，
    且**包间差 405×**（同为 2000 字符）。自己**不扫文本**的包（没有
    ``keyword_block`` / ``pattern_block``）在 L ≤ 2000 上被每次调用的固定开销
    盖住、看着近乎线性 —— 那是 R9(b) 与 R10 各拿掉了一个常数，**二次项没消失**。
    数字、复跑命令与读法见 ``bench/results/streaming.md``
    （``python3 bench/bench_streaming.py``）。
    别把某个 L 上的「每字符」当常数外推：这里曾写过「~0.07 ms/字符，10k 字符
    约 0.7 s」，那 0.07 是 L≈200 的瞬时值，10k 字符实为 ~10 s。
