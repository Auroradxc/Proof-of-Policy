...
class PoPCallbackHandler(BaseCallbackHandler):
    ...
    **跨 provider 不可比**，它变成了「抢在几个 chunk 内」而不是「抢在几个字符内」。
    ...
    而前缀长度随采样点线性增长 —— 实测 L 长 4 倍、每字符成本也长 ~3.7–4.0 倍，
    且**包间差 827×**（同为 2000 字符）。自己**不扫文本**的包（没有
    ``keyword_block``）在 L ≤ 2000 上被每次调用的固定开销盖住、看着近乎线性 ——
    那是 R9(b) 拿掉了一个常数，**二次项没消失**。数字、复跑命令与读法见
