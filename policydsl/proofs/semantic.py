...
# 模型契约（模型指纹 / 路径口径 / 图的字符上限）已下沉到
# :mod:`policydsl.core.model_fp`，这里**原地 re-import**。对象是同一个，所以
# ``sem.X`` 的调用方（测试、``scripts/prove/ezkl_prove.py``、``bench/``）一个字
# 都不用改。下沉的理由见该模块 docstring：原先 ``core`` 为了取指纹反过来
# import 本模块，是 ``core → proofs`` 的分层倒置。
from policydsl.core.model_fp import (
    ARTIFACT_NAMES,
    SEMANTIC_VERSION,
    SemanticError,
    cached_file_sha256,
