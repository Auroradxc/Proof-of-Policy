...
impl TempDir {
    ...
    /// Keeps the temporary directory (do not delete on drop) and returns its
    /// path. Equivalent to `into_path()`.
    ///
    /// This mirrors the API expected by consumers that rely on
    /// `TempDir::keep` (e.g. sp1-prover). Added locally as a patch; not part
    /// of the upstream `tempfile` crate.
    #[must_use]
