...
- 🔗 **轨迹绑定（P1-5）**：工具轨迹不再是 agent 自报的 `tool_calls`，而是**工具网关**签发的**回执链**（`policydsl/trace.py` + `pop-types::verify_receipt_chain`）。链**结构**由电路保证（删/换/重排 → `trace_unbound` fail-closed），**签发者身份**由链下 Ed25519 验签 + 公开值 `trace_root` 承担；`budget_bound(tokens)` 改为电路内自算。验收见 `tests/test_trace.py`（29 例：四条验收 + `verify_cert.py --receipts` 的第三方核对 + 截尾缺口用例）；边界如实标注于 `docs/security-model.md` §5
