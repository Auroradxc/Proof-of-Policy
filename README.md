...
> `bash scripts/demo/demo_all.sh` 把 8 条一次跑完并汇总成一份报告。
...
│  semantic/ ：确定性导出 ONNX → scripts/prove/ezkl_prove.py 出证                               │
...
│  Compose = (推理完整性 ∧ 策略合规)；驱动 scripts/prove/compose_proof.py                       │
└───────────────────────────────────────────────────────────────────────────────────────┘
                                      │
...
│  驱动 scripts/prove/prove_session.py；⚠️ 尾截断只有「根比对」拦得住，见 L8                      │
└───────────────────────────────────────────────────────────────────────────────────────┘
```
...
| # | 支路 | 驱动 | 在 `demo_e2e` | 前置 |
|---|---|---|---|---|
| ① | **公开模式主干**：流式(含早停) + MCP 工具 + 真 SP1 证明 + 挑战绑定 + **公私对比** + 账本 | `scripts/demo/demo_e2e.py` | ✅ 就是它 | — |
| ② | **私有模式**：响应承诺 + 逐违规证据承诺 + 脱敏证明 + 证据开示 | `scripts/demo/private_demo.py` | 🔶 部分（① 里已含「同一条 T 两种模式」的并排对比与证据开示；②里的六个专项实验仍只在 `private_demo.py`） | — |
| ③ | **语义规则**（P2-9）：ezkl 陪伴证明，验证方与 SP1 结论**合取** | `scripts/prove/ezkl_prove.py` | ⛔ | ezkl + `semantic/artifacts/` |
| ④ | **组合证明**（P1-6）：策略半 ∧ 推理半，两个 vkey | `scripts/prove/compose_proof.py` | ⛔ | — |
| ⑤ | **会话聚合**（P2-10）：一组证书的 Merkle 根 + 三条义务 | `scripts/prove/prove_session.py` | ⛔ | ① 的 `session.json` |
| ⑥ | **多证明者**（P2-11）：按规则类切三段，各角色自证自签 | `scripts/prove/prove_multiparty.py` | ⛔ | — |
| ⑦ | **链上锚定**（P7-c）：起 anvil → 部署 `Anchor.sol` → 摘要上链 | `scripts/anchor/anchor_e2e.sh` | ⛔（可选步骤） | foundry |
| ⑧ | **第三方独立验证**：只用公开产物复算全部结论（闭环） | `scripts/verify/verify_session.py` | ✅ 主 demo 末尾会提示这条 | ① 的产物 |
...
python3 scripts/demo/make_shots.py --run-demo
...
```bash
bash scripts/ops/retry_install_foundry.sh   # 装 foundry（anvil/cast）；已装则秒退
bash scripts/anchor/anchor_e2e.sh              # 起 anvil → 部署 Anchor.sol → 13 张证书摘要上链 → 第三方 --rpc 核对
...
```bash
python3 scripts/ops/proof_service.py --host-check   # 演示/边缘：秒级，证书如实标注 unproven
SP1_PROVER=cpu python3 scripts/ops/proof_service.py # 真证明：~2.5 分钟/次，峰值 ~10.2 GiB
...
- 🛠 **开发与使用手册**（动手改代码的入口）：**`docs/development.md`** —— 依赖分层（装到哪层能跑到哪层）、15 分钟跑通、改一处代码跑哪几条验证、常见任务配方、环境变量速查、产物地图、故障排查。与上面那篇的分工：`reproduce.md` 按**功能**讲「每个特性怎么复现出来」，这篇按**循环与角色**讲「平时怎么在这个仓库里干活」
...
- ⛓ 链上锚定（真跑本地 Anvil）：`contracts/Anchor.sol` + `bash scripts/anchor/anchor_e2e.sh`（部署 → 每张证书摘要上链 → 第三方 `verify_session --rpc` 核对 + 反例对照）；见 `docs/reproduce.md` §12
...
依赖（可选，安装后真实框架测试自动启用）：`pip install -r requirements-frameworks.txt`（或 `bash scripts/ops/install_frameworks.sh`）。
...
- **W6**：agent 插桩 + 合规证书 → `policydsl/adapters/agent.py` + 三个框架适配器 + `cert.py` + `scripts/demo/demo_e2e.py`
