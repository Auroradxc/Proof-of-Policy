...
> **同步状态（2026-09-12）**：§5 已补上 **L8**（跨证书一致性）与 **L9**（多证明者责任划分）
> 两条引理的摘要，§6/§7 的数字已与 `.tex` 对齐（469/13、30 例、三个 guest）。此后新增内容
> 仍以 `.tex` 为先 —— 本镜像只保证「不是错的」，不保证「是最新的」。
...
我们在单机 CPU 上实测：20k 字符 × 6 条规则的判定约 `1.1×10^8` 周期，真实 STARK 证明 ~**1–2 分钟**、
...
- **Cross-certificate（L8，P2-10）**：针对多轮会话（一次 agent run 产出一串**流式证书**），
  单张证书的正确性不足以说明「整条会话没被动过手脚」。`session` 域（**第三个 guest**，独立 vkey）
  把三条义务压进**一次**证明：① 每张被覆盖证书的 `policy_hash` 相同，且（给了策略包时）等于验证方
  **现场重编译**得到的那个；② 流式链**无缝**（`chain.index` / `chain.prev` 逐张咬合，中间没有缺口）；
  ③ 覆盖**完整**（交付的证书集就是被证明的那一组，不多不少）。做法是把每张证书规范载荷的**文本**
  哈希成叶子、用 **Merkle 根**聚合成一个承诺进公开值。三个设计要点每一个都是「换个做法就出漏洞」：
  **叶子取载荷文本的 `sha256`**（Rust 侧因此不需要任何 JSON 规范化，跨语言漂移在结构上不可能）；
  **Merkle 奇数末位「提升」而非「复制」**（复制会让 `[a,b,c]` 与 `[a,b,c,c]` 得到同一个根，
  于是「删掉链尾那张」多出一条伪造路径）；**尾截断只有 Merkle 根拦得住**（已实测）——`[0..k]` 前缀的
  `chain.index` / `prev` **依然连续**，电路内三条义务全部成立，所以**电路本身接受一个被砍过尾巴的证书集**，
  拦住它的是验证侧「证明承诺的根 vs 由**交付的**证书集重算的根」这一步；中间挖洞 / 换序则相反，
  `chain.index` / `prev` 当场就断，出不了证明。⚠️ **不保证**：电路内**不验网关签名**（zkVM 里没有网关公钥），
  它只断言「每张证书都带 seal」且 `keyid` 全同；覆盖范围**只有一个 run、且只有链上证书**。
  真·端到端实测 152.5 s（一次出证 + 一次独立验证 + 四条拒绝路径）。
- **Multi-prover（L9，P2-11）**：一条策略的不同规则，其证据本来就在**不同参与方**手上
  （内容类规则只看响应文本；工具类要读**网关**签发的回执链；语义规则要用**部署方**手里的模型与阈值），
  把整条策略交给一方去证等于要求它持有全部证据。多证明者把策略按**规则类**切成三段，三个角色各证一段、各签一段：

  | 角色 | 规则类 | 为什么是它 |
  |---|---|---|
  | 模型方 / `model` | keyword, normalized-keyword, pattern, format, length | 只看响应文本，不需要任何外部证据 |
  | 工具网关 / `gateway` | tool-arg, budget | 这两条要读 P1-5 的回执链，而链只有网关有 |
  | 部署方 / `deployer` | semantic | 它才持有模型与阈值（委托给陪伴证明，L7） |

  切片**两两不交、并集是全策略**；空切片**仍然要签名**——「这一段我不负责」必须是**显式**的。
  输出是一份 `multiparty.json`：共享的 `policy_hash` / `response_binding` / `trace_root`、一份 `plan`
  （角色 → {规则, `slice_sha256`}）及其摘要、$N$ 份切片证明、每个角色对自己那部分的 Ed25519 签名。
  ⚠️ **两点如实记下**：①「聚合证明」**不是递归聚合**——它是 $N$ 份切片证明（共享同一个 vkey）
  **加**一份拴住它们的证书，验证成本 $O(N)$；三个角色各持一把键只是**责任划分**，
  不是「不同角色用不同的电路」（切片对一个 guest 而言就是普通策略，因此**在盘的旧证明继续有效**）。
  ②「单角色切片被换」有三道拦网（part 自报的规则/摘要与 `plan` 对不上；改写 `plan` 会破坏另外两个角色的
  签名），**但三方合谋在签名层不可检出**——三把键一起改 plan、一起重签，签名层完全自洽，光看证书查不出来；
  拦住它的唯一办法是拿策略包**现场重编译**并与 `plan` 比对。这条边界被钉成一条会**通过**（不给策略包时）
  的用例，免得日后被读成「签名能挡住合谋」。

## 6. Implementation

Python 参考层（DSL/编译/NFA/私密/证书/锚定/框架适配）+ Rust（SP1 v6 workspace：`types` 共享判定，
**三个** guest —— `program`（策略域）、`infer-program`（推理域）、`session-program`（会话域），
各有一个入口断言自己的域，因此**三个不同的 `vkey`** —— 外加 `script` 驱动与 `verifier`）。

单测 + 集成测试 **469 全绿（13 skip 均为设计内）**。与本文各条义务对应的模块：

- `tests/test_semantic.py`（**30 例**）覆盖 P2-9 的语义规则委托（陪伴证明的逐字段绑定、四类换料攻击、私有模式 fail closed）；
- `tests/test_normalize.py`（28 例）覆盖 P2-9b 的折叠表与其 13 种非法声明；
- `tests/test_trace.py`（39 例）覆盖 §4.2.1 的四条验收与 P1-5b 的截尾对策
  （`TestSeal` + `test_tail_truncation_is_rejected`）；
- `tests/test_compose.py`（48 例）与 `tests/test_session.py`（38 例）分别覆盖 L6 与 L8，
  各含真·端到端用例（后者实测 152.5 s）；
- `tests/test_multiparty.py`（44 例）覆盖 L9 的切分、绑定与两条验收判据；
- `tests/test_rules_incircuit.py`（13 例）钉死 Python 参考层与电路内实现的逐点对齐。

`scripts/` 提供交叉验证、demo、证书签发/验证、会话聚合、组合、多证明者、截图；`docs/reproduce.md` 是复现指南。
...
多证明者当前**不覆盖三方合谋**（见 §5 的 L9 条）；切片共享**同一个** guest
（各持一把键只是责任划分，本仓库没有做「每个角色一条电路」）；
**递归聚合未做**，验证成本仍是 $O(N)$；
正则子集与 ASCII 语义扩展；证明开销优化（lookup/并行/预计算）；
与 zkAgent 轨迹证明、可验证 DP 的组合。

> 注：**同形异义折叠（P2-9b）已完成**（`NormalizedKeywordBlock`，全电路内、零依赖），
> 不再属于 Future Work —— 此前本文件把它列在这里是残留。
