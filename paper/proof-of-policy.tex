...
\textbf{三个} guest 程序（键分离，见 \S\ref{sec:security}）、组合驱动 \code{scripts/prove/compose_proof.py}
...
锚定支持两种后端（\code{policydsl/evidence/anchor.py}，上层接口一致）：\textbf{文件账本}（默认，
...
\code{scripts/demo/demo_e2e.py} 产生 14 张证书（流式链+早停、MCP 参数+响应、zk 证明）并锚定；
\code{scripts/verify/verify_session.py} 第三方仅凭公开产物验证：
\code{ledger_chain / signature / policy_hash / anchored / stream_chains / zk_proof} 全 PASS。

链上变体（\code{bash scripts/anchor/anchor_e2e.sh}：本地 Anvil $\to$ 部署 \code{Anchor.sol} $\to$
14 张证书摘要上链）：第三方 \code{verify_session --rpc} 逐证书 \code{anchoredAt} 读回并交叉核对
本地账本，\code{chain_anchored 14/14} PASS；反例对照（未登记摘要读回 0）确认该检查非恒真。
\code{--prove} 变体在真实 Core 证明（2.78 MB）下同样通过。

\paragraph{交叉验证（I1）。} \code{scripts/prove/cross_validate.py} 把 \textbf{19 条向量}同时交给
